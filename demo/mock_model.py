...
async def good_model(request: Request):
    """High-accuracy model: reads the sentiment hint from the input."""
    body = await request.json()
    content = body.get("messages", [{}])[0].get("content", "{}")
    try:
        row = json.loads(content.replace("'", '"'))
        label = row.get("sentiment", row.get("expected_label", row.get("label", "positive")))
...
async def bad_model(request: Request):
    ...
    try:
        row = json.loads(content.replace("'", '"'))
        correct = row.get("sentiment", row.get("expected_label", row.get("label", "positive")))
